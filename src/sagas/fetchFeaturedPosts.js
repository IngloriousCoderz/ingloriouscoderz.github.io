...
import { receiveFeaturedPostsList, receiveFeaturedPostsError, requestResource } from '../actions'
import fetch from './fetch'
import fetchResource from './fetchResource'
import { featuredPosts } from '../../blog-config'
...
export default function* fetchFeaturedPosts({ payload }) {
  try {
    ...
    if (list == null && !list.length) {
      ...
      list = JSON.parse(list).filter(({name}) => name.endsWith('.md'))
      .map(({name}) => ({
        id: name.slice(0, -3),
    ...
    const posts = yield list.map(({ id }) => call(fetchResource, requestResource('post', id)))
